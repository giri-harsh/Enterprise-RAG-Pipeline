...
API_HEADERS = {"X-API-Key": os.getenv("API_KEY")} if os.getenv("API_KEY") else {}
...
def _contexts_from_sources(sources: list) -> list[str]:
    """
    Flatten /query's `sources` into the plain strings RAGAS expects.

    The endpoint returns chunk dicts — {content, source, score, rerank_score} —
    since the API started carrying source attribution. RAGAS scores against raw
    text, so the content field is extracted here. The isinstance guard keeps older
    saved eval runs, which stored bare strings, loadable.
    """
    return [s["content"] if isinstance(s, dict) else str(s) for s in sources]
        ...
        for i, sample in enumerate(samples):
            ...
            with logfire.span(
                ...
                try:
                    ...
                    sample["actual_contexts"] = _contexts_from_sources(sources)[:5]
                    sample["actual_source_files"] = sorted(
                        {s["source"] for s in sources if isinstance(s, dict)}
                    )
