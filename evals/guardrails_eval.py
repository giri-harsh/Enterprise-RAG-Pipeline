...
def _is_blocked(response_json: dict) -> bool:
    ...
    Reads the explicit `blocked` flag the API now returns, falling back to the old
    thought_process string match so previously saved eval runs still parse.
    """
    if "blocked" in response_json:
        return bool(response_json["blocked"])
