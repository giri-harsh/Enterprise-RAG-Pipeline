...
try:
    ...
    # This module is usually the first thing a new clone runs, so a bare
    # ModuleNotFoundError here is the worst possible first impression. Say what
    # is actually wrong instead.
    _activate = r".venv\Scripts\Activate.ps1" if os.name == "nt" else "source .venv/bin/activate"
...
# Without a LOGFIRE_TOKEN this used to raise before indexing a single document.
configure_logfire("enterprise-ingestion-service")

# Local folder where parsed + chunked JSON metadata is saved (replaces GCS processed bucket)
PROCESSED_DATA_DIR = "processed_data"

# Reuses the lazily-built client from the retrieval service rather than opening a
# second connection to the same cluster.
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Embedded Qdrant holds an exclusive lock on its data directory, so the API
        # server and this CLI cannot both be running. The library's own message
        # ("already accessed by another instance") does not say what to do about it.
        if "already accessed" in str(exc).lower():
            print(
                "\nThe local Qdrant directory is locked by another process.\n"
                "Stop the API server (Ctrl+C in the window running scripts/demo.py), "
                "then run this again.\n"
            )
            sys.exit(1)
        raise
    ...
    print("\nIngestion complete. Start the demo with:  python scripts/demo.py")
