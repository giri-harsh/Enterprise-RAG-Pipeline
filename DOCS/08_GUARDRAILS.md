...
> **This project declares no `models:` block.** `rails.py` passes the LLM directly
> — `LLMRails(config, llm=guard_llm)` with ChatGroq `llama-3.1-8b-instant` — and an
> injected llm takes precedence over anything declared in YAML. The block below is
> shown because you will see it in NeMo's own examples, but carrying it here meant
> the config stated the guardrail ran on an OpenAI model when it never did, and
> implied an `OPENAI_API_KEY` this project does not use. It was removed rather than
> corrected: the single source of truth for the guardrail model belongs in
> `rails.py`, not duplicated in a string.

```yaml
# Illustrative only — NOT used in this project.
models:
  - type: main
    engine: openai       # overridden by the llm= constructor arg anyway
    model: gpt-3.5-turbo

# What colang_rules.py actually declares:
instructions:
  - type: general
    content: |
